...
<br/>
	...
	<% 
       ...
       <br>
       <c:if test="${requestScope.totalCount>1 }">
  
		<a href="showallbook.jsp?ppage=${requestScope.ppage.currentPage-1 }">上一页</a>
		<c:forEach begin="1" end="${requestScope.ppage.totalPage }"  var="p">
		<a href="showallbook.jsp?ppage=${p }">${p }</a> 
		</c:forEach>
		<a href="showallbook.jsp?ppage=${requestScope.ppage.currentPage+1 }">下一页</a>
       </c:if>
       </c:if>
       <c:if test="${sessionScope.user.utype=='1' }">抱歉，您暂无权限查看该页面！</c:if>
       </c:if>
